...
## Participants
Children from age `r (min(demo_data$age))/365 ` to `r (max(demo_data$age))/365` (M = `r (mean(demo_data$age))/365`) and their parents were recruited from a databse of families in the local community or approached on the floor of a local science museum in order to achieve a planned sample of 40 parent-child dyads. Five dyads were recruited as pilot participants, and their data were not analyzed. A total of 47 parent-child pairs were recruited for the study, but data from six pairs were dropped from analysis due to experimental error or failure to complete the study. The presented data are from the remaining 41 children (`r length(demo_data[demo_data$gender == "f"])` females). There were `r length(demo_data[demo_data$race == "White"])` white participants, `r length(demo_data[demo_data$race == "Black or African American"])` Black participants, `r length(demo_data[demo_data$race == "Asian"])` Asian participants, and `r length(demo_data[demo_data$race == "Other"])` participants who were mixed-race or declined to state their race.
...
A modified version of the MacArthur-Bates Communicative Development Inventory [CDI; @fenson2007], a parent-reported measure of children’s vocabulary, was administered before the testing session via an online survey. The selected animal words were embedded among the 85 words in the survey. Two of the animal words--one in the early AOA and one in the late AOA category were excluded by accident, and so trials for those words were not included in analysis.
...
```{r ipads, fig.env = "figure", fig.pos = "tb", fig.align='center', fig.width=2, fig.height=2, out.width = "200px", set.cap.width=T, num.cols.cap=1, fig.cap = "Example of child's iPad display (top) and parent's iPad display (bottom) during the experiment."}
...
```{r length_fig, set.cap.width=T, num.cols.cap=1, fig.cap = "Parent utterance length on the first (blue) and second (red) appearance of targets. The x-axis is proportion of parents who responded that their children knew the word.", fig.height = 2}
