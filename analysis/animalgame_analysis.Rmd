...
targets <- tidy_data %>%
  distinct(target) %>%
  filter(!is.na(target), !target %in% c("n/a", "unknown", "cat>", "hold on",
                                        "rhinoceorus")) %>%
  pull()
...
tidy_vocab %>%
  ...
  mutate(type = if_else(word %in% c("apple", "artichoke"), "practice",
         if_else(word %in% c("cat", "dog", "bird", "cow", "fish", "duck", "horse", "elephant"),
                 "early", "late"))) %>%
  write_feather("data/word_difficulty.feather")
...
target_data %>%
  select(subj, person, id, target, trial, phase, trial_target, appearance, length, difficulty, understands, correct, trial_type, game_trial) %>%
  write_feather("data/target_data.feather")
...
ggplot(test_data, aes(x = ease, y = correct)) +
