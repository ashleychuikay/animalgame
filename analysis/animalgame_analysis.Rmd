...
```{r word_difficulty}
jpeg("word_diff.jpeg", width = 14, height = 10, units = "in", res = 600)
...
ggplot(plotting_words, aes(x = word, y = understands, ymin = ci_lower, 
  ...
  geom_pointrange() +
  labs(y = "Proportion of children who know the word (parent report)", x = "Word")
dev.off()
```
...
pre_data <- target_data %>%
  filter(phase == "pre") %>%
  summarise(length = sum(length)) %>%
  group_by(phase, understands, id, trial_target) %>%
  summarise(length = sum(length, na.rm = T)) %>%
  summarise(length = mean(log(length+1), na.rm = T)) %>%
  tidyboot_mean(length) %>%
  mutate(understands = factor(understands, labels = c("Child Doesn't Know", "Child Knows")))
...
jpeg("lag_data.jpg", width = 6, height = 4, units = "in", res = 600)
target_data %>%
  ungroup() %>%
  mutate(understands = factor(understands, labels = c("Child doesn't know", "Child knows"))) %>%
...
#  mutate(phase = factor(as.character(phase), levels = c("pre", "post"))) %>%
  ...
  group_by(id, phase, understands, trial_target) %>%
  mutate(lag_correct = lag(correct)) %>%
  filter(appearance == "second", phase == "pre") %>%
  group_by(phase, understands, lag_correct, id, trial_target) %>%
  summarise(length = sum(length, na.rm = T)) %>%
  summarise(length = mean(log(length+1), na.rm = T)) %>%
  filter(!is.na(understands)) %>%
