...
```{r write_word_difficulty, eval = F}
...
subj_vocab <- tidy_vocab %>%
  ...
  left_join(total_vocab, by = "subj") 

write_feather(subj_vocab, here("data/subj_vocab.feather"))
```
...
target_data_gaps <- parent_data %>%
 ...
 # filter(phase != "during") %>%
  group_by(phase, avg_known, understands, appearance, trial_target, id)
...
target_data <- target_data_gaps %>%
  select(-subj) %>%
  left_join(ids) %>%
  left_join(response_data, by = c("subj", "trial_target" = "target", 
                                  "trial" = "trial_num")) %>%
  filter(trial_type != "practice")

target_data %>%
  ungroup() %>%
  select(subj, person, target, trial, phase, trial_target, appearance, length, avg_known, understands, correct, trial_type, game_trial) %>%
