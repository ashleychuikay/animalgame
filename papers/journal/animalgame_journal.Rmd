...
ggplot(plotting_words, aes(x = word, y = empirical_stat, ymin = ci_lower, 
  ...
  labs(y = "Parents report child knows", x = "") 
```
...
known_lmer <- trial_known_data %>%
  ...
  lmer(length ~ phase * understands + (phase + understands | subj) + 
         (1 | trial_target), control = lmerControl(optimizer = "bobyqa"), 
       data = .) %>%
  tidy() %>%
  filter(effect == "fixed") %>%
  select(-effect, -group)
...
known_difficulty_lmer <- trial_known_data %>%
  ...
  mutate(difficulty = 1 - difficulty, 
         understands = factor(understands, levels = c("known animal", 
                                                      "unknown animal"))) %>%
  lmer(length ~ difficulty + understands + (difficulty + understands |subj) +
         (1|trial_target), 
       data = .) %>%
  tidy() %>%
  filter(effect == "fixed") %>%
